...
dirA<-paste("D:/Method 4/",sysA,"/", sep ="")
...
while(c <= length(sysB)) {
  ...
  dirB<-paste("D:/Method 4/",sysA,"/",sysB[c],"/", sep= "")
  dir.create(dirB, showWarnings = TRUE, recursive = FALSE)
  
  pathoutputt<-paste("D://Method 4/",sysA,"/",sysB[c],"/","t-test.txt",sep="")
  pathoutputp<-paste("D://Method 4/",sysA,"/",sysB[c],"/","p-values.txt",sep="")
  ...
  while (topic <= 500 ) {
    ...
    x1 <- rep(0, 50-length(fc))    
    x2 <- rep(0, 50-length(sc))
