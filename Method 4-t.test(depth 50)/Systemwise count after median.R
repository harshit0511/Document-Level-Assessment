...
while(c<=length(sysA)) {
  ...
  pathinput<-paste("D://Method 4/",sysA[c],"/Final Count.txt", sep = "")
  counttxt<-read.delim(file=pathinput, header=FALSE)
  
  pathoutput<-paste("D://Method 4/Final System Count.txt", sep = "")
  
  firstrow<-counttxt[1,]
  write.table(firstrow,file=pathoutput, row.names = F, col.names = F, append=T,quote=F)
