...
dirA<-paste("D:/Method 3/",sysA,"/", sep ="")
...
while(c <= length(sysB)) {
  ...
  pathoutputt<-paste("D://Method 3/",sysA,"/",sysB[c],"/","t-test.txt",sep="")
  pathoutputp<-paste("D://Method 3/",sysA,"/",sysB[c],"/","p-values.txt",sep="")
  
  dirB<-paste("D:/Method 3/",sysA,"/",sysB[c],"/", sep= "")
  ...
  while (topic <= 500 ) {
    ...
    t<-try(t.test(a1,a2, paired=TRUE,alternative ="greater",conf.level=0.99),silent=TRUE)
