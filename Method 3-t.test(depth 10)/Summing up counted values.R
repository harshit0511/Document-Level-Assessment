...
pathoutput<-paste("D://Method 3/",sysA,"/countALL.txt", sep = "")

counts<-0
e<-1
while(e<=length(sysB)){
  
  pathcount<-paste("D://Method 3/",sysA,"/",sysB[e],"counts.txt", sep = "")
  countfile<-read.table(pathcount, quote="\"")
  secondcount<-countfile[,4]
...
write(paste("System:",sysA,"\tCount:",counts,"\tTotal:",total,"\tPercentage:",countperc),file=pathoutput)
