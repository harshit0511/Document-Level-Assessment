...
while(c<=length(sysA)) {
  count<-0
  pathinput<-paste("D://Method 5/",sysA[c],"/P-values.txt", sep ="")
  Pvalue<-read.delim(file=pathinput, header=FALSE)
  pathoutput<-paste("D://Method 5/",sysA[c],"/Final Count.txt", sep = "")
  thirdcol<-Pvalue[,3]
  ...
  total<-length(thirdcol)
  countperc<-(count/total)*100
  write(paste("System:",sysA[c],"\tCount:",count,"\tTotal",total,"\tPercentage:",countperc),file=pathoutput)
