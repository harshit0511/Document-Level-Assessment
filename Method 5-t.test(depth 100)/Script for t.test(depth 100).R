...
dirA<-paste("D:/Method 5/",sysA,"/", sep ="")
...
while(c <= length(sysB)) {
  ...
  dirB<-paste("D:/Method 5/",sysA,"/",sysB[c],"/", sep= "")
  dir.create(dirB, showWarnings = TRUE, recursive = FALSE)
  
  pathoutputt<-paste("D://Method 5/",sysA,"/",sysB[c],"/","t-test.txt",sep="")
  pathoutputp<-paste("D://Method 5/",sysA,"/",sysB[c],"/","p-values.txt",sep="")
  ...
  while (topic <= 500 ) {
    ...
    x1 <- rep(0, 100-length(fc))    
    x2 <- rep(0, 100-length(sc))
    
    a1 <- c(fc, x1)
    a2 <- c(sc, x2)
    
    t <- try(t.test(a1,a2, paired=TRUE, alternative = "greater", var.equal = FALSE), silent=T)
