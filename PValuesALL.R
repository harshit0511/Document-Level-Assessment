...
while(c<=length(sysA)) {
  ...
  pathinput<-paste("D://Method 1/",sysA[c],"/P-Values.txt", sep = "")
  pvalues<-read.delim(file=pathinput, header=FALSE)
  
  pathoutput<-paste("D://Method 1/PValuesALL.csv", sep = "")
  
  row<-pvalues
  write.table(row,file=pathoutput, row.names = F, col.names = F, append=T,quote=F, sep=",")
