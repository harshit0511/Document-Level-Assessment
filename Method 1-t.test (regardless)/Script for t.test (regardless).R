...
dirA<-paste("D:/Method 1/",sysA,"/", sep ="")
...
while(c <= length(sysB)) {
  ...
  pathoutputt<-paste("D://Method 1/",sysA,"/",sysB[c],"/","t-test.txt",sep="")
  pathoutputp<-paste("D://Method 1/",sysA,"/",sysB[c],"/","p-values.txt",sep="")

  dirB<-paste("D:/Method 1/",sysA,"/",sysB[c],"/", sep= "")
  ...
  while (topic <= 500 ) {
    ...
    if(is.null(fourthcol)){
      write(paste("Topic:",topic,"\t System:",sysB[c]),file="D://0 matches.txt",append=TRUE)
    }
    else if(length(fourthcol)==1){
      write(paste("Topic:",topic,"\t System:",sysB[c]),file="D://1 matches.txt",append=TRUE)
    }
    else{
    t<-try(t.test(fourthcol,seventhcol, paired=TRUE,alternative ="greater",conf.level=0.99),silent=TRUE)
    p<-try(t.test(fourthcol,seventhcol, paired=TRUE,alternative ="greater",conf.level=0.99)[["p.value"]],silent=TRUE)
