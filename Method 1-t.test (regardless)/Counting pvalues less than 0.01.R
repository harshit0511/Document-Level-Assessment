...
sysB <- c("CWI0010","Flab9atN","Flab9atd2N","Flab9atdN","Flab9atdnN",
...
sysC <- c("CWI0000","CWI0001")

c<-1
while(c<=length(sysB)) {
  count<-0
  pathinputA<-paste("D://Method 1/",sysA,"/",sysB[c],"/p-values.txt", sep ="")
  Pvalue<-read.table(file=pathinputA, quote="\"")
  pathoutputsysA<-paste("D://Method 1/",sysA,"/",sysB[c],"counts.txt", sep = "")
  SecondcolA<-Pvalue[,2]
...
while(e<=length(sysC)) {
  count<-0
  pathinputB<-paste("D://Method 1/",sysC[e],"/",sysA,"/p-values.txt", sep ="")
  PvalueB<-read.table(file=pathinputB, quote="\"")
  pathoutputsysB<-paste("D://Method 1/",sysA,"/",sysC[e],"counts.txt", sep = "")
